...
import type { StationsFile, StationEntry, RecipesFile, RecipeEntry } from './types.js';
...
export function getAllCraftableItemIds(stations: StationsFile, stationType: number): string[] {
  const station = getStation(stations, stationType);
  if (!station) return [];
...
export function getStationTiers(stations: StationsFile, stationType: number): number[] {
  const station = getStation(stations, stationType);
  if (!station) return [];
  return Object.keys(station.tiers)
    .map(Number)
    .sort((a, b) => a - b);
}
...
export function getStationSummary(stations: StationsFile) {
