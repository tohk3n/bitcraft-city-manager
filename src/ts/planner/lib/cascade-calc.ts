...
function getOrThrow<K, V>(map: Map<K, V>, key: K): V {
  ...
  if (value === undefined) {
    throw new Error(`Map missing expected key: ${key}`);
  }
  return value;
}
...
function buildNode(
  ...
  const key = createKey(node.name, node.tier);
  const item = getOrThrow(items, key);
  ...
  const status: NodeStatus = satisfied ? 'complete' : contribution > 0 ? 'partial' : 'missing';
