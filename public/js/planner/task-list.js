...
function renderCards(container) {
    let items = currentItems.filter(item => {
        if (hideComplete && item.deficit === 0)
            return false;
        if (filterTier && item.tier !== parseInt(filterTier, 10))
            return false;
        if (filterActivity && categorizeByActivity(item.name) !== filterActivity)
            return false;
        return true;
    });
...
function sort(items, by) {
    ...
    switch (by) {
        ...
        case 'activity':
            return sorted.sort((a, b) => categorizeByActivity(a.name).localeCompare(categorizeByActivity(b.name)) || b.deficit - a.deficit);
        case 'name':
