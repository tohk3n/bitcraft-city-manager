...
export function expandRecipes(codex, targetCount, mappings = null) {
    return {
        ...
        tier: codex.tier,
        targetCount,
        researches: codex.researches.map(research => expandNode(research, targetCount, mappings))
    };
}
/**
...
function expandNode(node, batchCount, mappings) {
    ...
    return {
        ...
        children: (node.children || []).map(child => expandNode(child, batchCount, mappings))
    };
}
/**
